...
def process_json_with_embeddings(
    ...
    if store_in_db and "embeddings" in enhanced_data:
        ...
        try:
            ...
            doc_id = db_manager.store_document_with_embeddings(
                enhanced_data, collection_name
            )
...
def scrape_and_process(
    url: str, store_in_db: bool = False, collection_name: str = "scraped_content"
) -> Dict[str, Any]:
    """
...
def search_website_data(
    query_text: str, collection_name: str = "scraped_content", limit: int = 5
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            for i, doc in enumerate(all_results["documents"]):
                ...
                description = (
                    metadata.get("description", "").lower() if metadata else ""
                )
            ...
            if matching_docs:
                ...
                for doc in matching_docs:
                    formatted_result = {
                        ...
                        "content_snippet": (
                            doc["document"][:200] + "..." if doc["document"] else ""
                        ),
...
def main():
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process JSON data with embeddings"
    )
    ...
    process_parser.add_argument(
        "--collection", default="scraped_content", help="Database collection name"
    )

    # Scrape URL command
    scrape_parser = subparsers.add_parser(
        "scrape", help="Scrape URL and process with embeddings"
    )
    ...
    query_parser = subparsers.add_parser(
        "query", help="Query vector database for similar content"
    )
    ...
    try:
        if args.command == "process":
            ...
            result = process_json_with_embeddings(
                json_data, args.store, args.collection
            )
